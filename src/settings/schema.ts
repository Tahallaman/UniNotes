...
export const SETTING_FIELDS: readonly SettingField[] = [
  ...
  {
    path: "highlights.finishSentenceSeconds",
    label: "Run on to finish the sentence (up to, seconds)",
    group: "Highlights",
    type: "int",
    min: 0,
    max: 60,
    dependsOn: "highlights.enabled",
    help: "A cue boundary is a breath, not a full stop, so snapping to one still cuts the lecturer off mid-clause. The transcript is punctuated, so a span carries on to the next full stop as long as it is within this. If none is, the span stops where it was. 0 turns it off.",
  },
  {
    path: "highlights.joinGapSeconds",
    label: "Join spans closer than (seconds)",
    group: "Highlights",
    type: "int",
    min: 0,
    max: 30,
    dependsOn: "highlights.enabled",
    help: "Spans this close together play as one continuous stretch, so they are stored as one, with both reasons kept. The prompt asks for this too; this is the backstop.",
  },
  {
    path: "highlights.maxSegmentSeconds",
    label: "Longest a span may ever be (seconds)",
    group: "Highlights",
    type: "int",
    min: 10,
    max: 900,
