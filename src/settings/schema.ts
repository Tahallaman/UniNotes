...
export const SETTING_FIELDS: readonly SettingField[] = [
  ...
  },
  {
    ...
    help: "A ceiling, not a target. A share rather than a number of minutes, because ten minutes is most of a lab and nothing of a two-hour lecture.",
  },
  {
    path: "highlights.presets.skim.minWeight",
    label: "Skim — lowest score kept",
    group: "Highlights",
    type: "int",
    min: 1,
    max: 5,
    dependsOn: "highlights.enabled",
    help: "Every span is scored 1–5. 5 keeps only what the lecture can't be understood without.",
  },
  {
    ...
    max: 100,
    dependsOn: "highlights.enabled",
    help: "The middle preset, and the one the button is named after.",
  },
  {
    path: "highlights.presets.highlights.minWeight",
    label: "Highlights — lowest score kept",
    group: "Highlights",
    type: "int",
    min: 1,
    max: 5,
    dependsOn: "highlights.enabled",
    help: "4 and up: the central ideas and the worked examples.",
  },
  {
    ...
    max: 100,
    dependsOn: "highlights.enabled",
    help: "Still a ceiling. A mostly-admin lecture gives a short Deep, because there was nothing else in it.",
  },
  {
    path: "highlights.presets.deep.minWeight",
    label: "Deep — lowest score kept",
    group: "Highlights",
    type: "int",
    min: 1,
    max: 5,
    dependsOn: "highlights.enabled",
    help: "Lower this to 2 to include the marginal spans as well.",
  },
  {
    path: "highlights.minSegmentSeconds",
    label: "Shortest span (seconds)",
    group: "Highlights",
    type: "int",
    min: 5,
    max: 300,
    dependsOn: "highlights.enabled",
    help: "Anything shorter is a jump cut rather than a span, and a reel of those is unwatchable.",
  },
  {
